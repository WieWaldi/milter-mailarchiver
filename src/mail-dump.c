//  MailArchiver RZ-Amper Fork Version 0.1, 2013
	...
	char **envfrom;
{
	...
	if (settings_timestamp)	//user want add timestamp & username into file name
	  ...
	  strftime(currtime_buf,sizeof(currtime_buf),"%Y%m%d_%H-%M",currtime_ts); /* Format time, "yyyy-mm-dd-hh-mm" */
	...
	if (settings_timestamp)
	  sprintf(priv->mlfi_fname,"%s/%s_%s__XXXXXXXX",settings_outputdir,currtime_buf,email_from_checked);
	else
	  sprintf(priv->mlfi_fname,"%s/XXXXXXXX",settings_outputdir);
	...
	else if (ok)
	{
		...
		//now - first we must rename file
		  ...
		  if (p == NULL)
		    ...
		    p++;
		  snprintf(hbufa, sizeof hbufa, "%s@%s by %s", p, host, VERSION);
		  smfi_addheader(ctx, "X-Archived", hbufa);
          snprintf(hbufb, sizeof hbufb, "Get in contact with postmaster@rz-amper.de about your archived mails");
          smfi_addheader(ctx, "X-Archived-Info", hbufb);
		  }
#if defined(USE_SYSLOG)
...
{
	"MailArchiver. writted by WieWaldi",	/* filter name */
...
void program_printhelp() {
    printf("option:\r\n");
    printf("-h          this help\r\n");
    printf("-d          specify directory for dump mail\r\n");
    printf("-s          specify socket file for mail server\r\n");
    printf("-i          interactive work, not going into background\r\n");
    printf("-t          add timestamp & sendername into filename\r\n\r\n\r\n");
}

void program_printinfo() {
    printf("\r\nmail-dump.solaris         %s\r\n", VERSION);
    printf("Copyright (C) 2013        Rechenzentrum Amper\r\n");
...
int main(int argc, char **argv) {
	...
	while ( (c = getopt(argc, argv, "ithd:s:")) != -1)
	{
		switch (c)
		{
		    case 's':
